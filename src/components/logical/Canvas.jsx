...
export default function Canvas({ projectId }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {/* ── Toolbar overlay ──────────────────────────────────────── */}
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-white rounded-2xl shadow-lg border border-slate-200 px-3 py-2">
        {/* Add idea */}
        <button
      ...
      {selectedId && (() => {
        ...
        if (!idea) return null
        return (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-white rounded-2xl shadow-lg border border-slate-200 px-3 py-2">
            <button
              onClick={() => setEditingId(selectedId)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-100 rounded-xl transition-colors font-medium"
            >
              <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
